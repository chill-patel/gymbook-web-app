...
import { z } from 'zod';
import type { MemberPackage } from '@/api/types';
...
export default function EditPlanDialog({ open, onClose, onSave, plan }: Props) {
  const {
    register,
    handleSubmit,
    reset,
  ...
  useEffect(() => {
    if (open && plan) {
      reset({
        ...
        admissionFees: String(plan.admissionFees ?? 0),
        paymentMethod: 'Cash',
  ...
  const admissionFeesVal = watch('admissionFees');

  const priceSummary = useMemo(() => {
    const base = plan.totalAmount;
    const admission = Number(admissionFeesVal) || 0;
    ...
    const totalAfterDiscount = Math.max(0, base + admission - discountAmount);
    const dueAmount = Math.max(0, totalAfterDiscount - paid);
    return { base, admission, discountAmount, totalAfterDiscount, dueAmount };
  }, [plan.totalAmount, paidVal, discountVal, discountType, admissionFeesVal]);
